  ...
  return (
    <div className="space-y-12">
      <section className="text-center py-20">
        <h1 className="text-5xl md:text-7xl font-heading font-bold">Welcome, {user?.name || 'User'}!</h1>
      ...
      <section>
        {loading ? <p>Loading statistics...</p> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <KpiCard title="Total Students" value={kpis.totalStudents.toLocaleString()} icon={<UsersIcon />} />
            <KpiCard title="Faculty Members" value={kpis.totalFaculty.toLocaleString()} icon={<AcademicCapIcon />} />
            <KpiCard title="Active Courses" value={kpis.totalCourses.toLocaleString()} icon={<BookOpenIcon />} />
          </div>
        )}
      </section>
    </div>
