...
const AnalyticsPage: React.FC = () => {
  ...
  useEffect(() => {
    api.getAnalyticsData()
      .then(data => { setAnalyticsData(data as any); setLoading(false); })
  ...
  const tooltipStyle = { backgroundColor: theme === 'dark' ? '#303030' : '#ffffff', border: `1px solid ${gridStrokeColor}` };
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={analyticsData.gradeDistribution} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
                {analyticsData.gradeDistribution.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
              </Pie>
              <Tooltip contentStyle={tooltipStyle} />
              <Legend />
            </PieChart>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={analyticsData.attendanceSummary}>
            ...
            <Bar dataKey="value" name="Records">
              {analyticsData.attendanceSummary.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
