...
const MonthNavigator: React.FC<{ currentDate: Date; onMonthChange: (offset: number) => void; }> = ({ currentDate, onMonthChange }) => {
  const today = new Date();
  const semesterStartDate = new Date(today.getFullYear(), today.getMonth() - (SEMESTER_MONTH_DURATION - 1), 1);
  const canGoPrevious = currentDate >= new Date(semesterStartDate.getFullYear(), semesterStartDate.getMonth() + 1, 1);
  const canGoNext = currentDate < new Date(today.getFullYear(), today.getMonth(), 1);

  return (
    <div className="flex justify-between items-center mb-4">
      <button onClick={() => onMonthChange(-1)} disabled={!canGoPrevious} className="bg-stone-200 dark:bg-stone-700 px-4 py-2 rounded-md disabled:opacity-50">&lt; Prev</button>
      <h2 className="text-xl font-bold font-heading">{currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</h2>
      <button onClick={() => onMonthChange(1)} disabled={!canGoNext} className="bg-stone-200 dark:bg-stone-700 px-4 py-2 rounded-md disabled:opacity-50">Next &gt;</button>
    </div>
  );
};
...
const AttendanceCalendarGrid: React.FC<{
  students: Student[]; attendance: any; currentDate: Date; isEditable: boolean; onStatusChange: (studentId: string, date: string) => void;
          ...
          {students.map(student => (
            <tr key={student.id} className="border-t border-stone-200 dark:border-stone-700">
              ...
              {[...Array(daysInMonth).keys()].map(i => {
                ...
                if (isWeekend) return <td key={day} className="p-0 bg-stone-50 dark:bg-stone-800/50"></td>;
                return (
                  <td key={day} className="p-0">
                    <button onClick={() => onStatusChange(student.id, dateStr)} disabled={!isEditable}
...
const AttendancePage: React.FC = () => {
  ...
  useEffect(() => {
    api.getCourses().then(data => { setCourses(data); if (data.length > 0) setSelectedCourse(data[0].id); });
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([api.getStudentsByCourse(selectedCourse), api.getDailyAttendanceForCourse(selectedCourse, year, month)])
      .then(([studentData, attendanceData]) => {
        if (user?.role === 'Student' && user.studentId) setStudents(studentData.filter(s => s.id === user.studentId));
  ...
  const handleStatusChange = useCallback((studentId: string, date: string) => {
    ...
    api.updateAttendance(selectedCourse, studentId, date, nextStatus).catch(() => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        {isFacultyOrAdmin && (
          <button onClick={() => setIsUpdateMode(!isUpdateMode)} className="bg-[#CF5256] text-white font-bold py-2 px-4 rounded-md">
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-stone-800/50 p-6 rounded-lg border">
          <label htmlFor="course-select" className="block text-sm font-medium">Select Course</label>
          <select id="course-select" value={selectedCourse} onChange={e => setSelectedCourse(e.target.value)}
            className="mt-1 block w-full md:w-1/2 bg-stone-100 dark:bg-stone-800 border rounded-md py-2 px-3 focus:outline-none">
        ...
        <div className="bg-white dark:bg-stone-800/50 p-6 rounded-lg border">
          ...
          <AttendanceLegend />
          {loading ? <div className="text-center p-8">Loading attendance...</div> : selectedCourse && students.length > 0 ? (
            <AttendanceCalendarGrid students={students} attendance={attendance} currentDate={currentDate} isEditable={isEditable} onStatusChange={handleStatusChange} />
